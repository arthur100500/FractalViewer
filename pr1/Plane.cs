...
namespace FractalRenderer
{
    public class Plane
    {
        ...
        public Plane(float[] coordinates, Shader shader, Texture texture)
        {
            ...
            _shader = shader;
            _texture = texture;
            zoom_info = new[] { _vertices[0], _vertices[15], _vertices[1], _vertices[6] };
        }
        ...
        public float[] GetRelativeCursorPosition(Window window, int x, int y)
        {
            ...
            var xpos = -1 + (-window.Location.X + x - 8) / (float)window.Width * 2;
            var ypos = 1 - (-window.Location.Y + y - 30) / (float)window.Height * 2;
        ...
        {
            float[] diff = { cursor_coords2[0] - cursor_coords1[0], cursor_coords2[1] - cursor_coords1[1] };
        ...
        {
            if (zoom_limited) return n <= 1.0f && n >= 0.0f;
            return true;
        }
    }
}